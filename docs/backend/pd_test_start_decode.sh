...
if [ "${PYTHON_VERSION}" != "3.10" ]; then
  ...
  echo "You can override explicitly, for example: PYTHON_BIN=/opt/conda310/bin/python sh docs/backend/pd_test_start_decode.sh" >&2
  exit 1
fi
...
echo "Starting DECODE node with:"
echo "  ${PYTHON_BIN} -m rtp_llm.start_server --checkpoint_path=${CHECKPOINT_PATH} --model_type=${MODEL_TYPE} --role_type=DECODE --start_port=${DECODE_PORT} --use_local=${USE_LOCAL} --remote_rpc_server_ip=${PREFILL_HOST}:${PREFILL_PORT}${TOKENIZER_PATH:+ --tokenizer_path=${TOKENIZER_PATH}}"
